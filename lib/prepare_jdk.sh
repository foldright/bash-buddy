...
showInfoOfSdkman() {
    ...
    blueEcho "sdk installed by SDKMAN:"
...
switchToJdk() {
    ...
    if isNumber "$1"; then
        ...
        [ -n "$JAVA_HOME" ] || die "JAVA_HOME of java version $1($javaHomeVarName) is unset or blank: $JAVA_HOME"
    else
        # set by java home
        export JAVA_HOME="$1"
        [ -n "$JAVA_HOME" ] || die "jdk $1 is blank: $JAVA_HOME"
    fi
    ...
    [ -f "$java_cmd" ] || die "\$JAVA_HOME/bin/java ($java_cmd) is NOT found!"
    [ -x "$java_cmd" ] || die "\$JAVA_HOME/bin/java ($java_cmd) is NOT executable!"
}
